...
public class ShopItemSource extends Source{
	public ShopItemSource(Actor actor) {
		super(actor);
	}

	public Payload dragStart(InputEvent event, float x, float y, int pointer) {
		Sprite item = TwoD.getShop().shopItems[((ShopItemActor)super.getActor()).positionId].getSprite();
		if (item != null) {
			Payload payload = new Payload();
			payload.setDragActor(new DragActor(item));
			payload.setObject(new DragObject(TwoD.getShop().shopItems[((ShopItemActor)super.getActor()).positionId].getItemId(), ((ShopItemActor)super.getActor()).positionId, "ShopItemSource"));
			TwoD.getShop().shopItems[((ShopItemActor)super.getActor()).positionId].setSprite(null);
			TwoD.getShop().shopItems[((ShopItemActor)super.getActor()).positionId].setItemId(0);
			return payload;
		}
		return null;
	}
	...
	public void dragStop(InputEvent event, float x, float y, int pointer, Payload payload, Target target) {
		...
		if (target != null && target.getClass() == InventoryItemTarget.class) {
			...
			if (TwoD.getInventory().items[targetActor.positionId].getItemId() == 0 && TwoD.getInventory().items[targetActor.positionId].getSprite() == null && (TwoD.getPlayer().getMoney() - TwoD.getItems().itemsSellPrice[dragObject.itemId]) > 0) {
				...
				TwoD.getPlayer().setMoney(TwoD.getPlayer().getMoney() - TwoD.getItems().itemsSellPrice[dragObject.itemId]);
			}else{
